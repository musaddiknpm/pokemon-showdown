...
import { Economy } from '../../impulse/modules/economy';
...
export class Tournament extends Rooms.RoomGame<TournamentPlayer> {
	...
	getRewards() {
		...
		return {
			winner: Math.floor(10 * multiplier),
			runnerUp: Math.floor(6 * multiplier),
		};
	}
			...
			try {
				...
				if (rewardConfig?.eligibleRooms.includes(this.room.roomid)) {
					...
					const isSingleElimination = this.generator.name?.toLowerCase().includes('elimination') &&
						(this.generator.maxSubtrees === 1 || /single/i.test(this.generator.name));
