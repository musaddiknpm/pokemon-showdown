...
export class PGTable<T extends Record<string, any>> {
	...
	protected buildWhere(
		...
		for (const key of keys) {
			...
			if (value === null) {
				...
				const opKeys = Object.keys(value as WhereOperator) as (keyof WhereOperator)[];
	...
	async insert(data: Partial<T>, returning: string = '*', options?: BaseOptions): Promise<T | null> {
	...
	async insertMany(dataArray: Partial<T>[], returning: string = '*', options?: BaseOptions): Promise<T[]> {
	...
	async truncate(cascade: boolean = false, options?: BaseOptions): Promise<void> {
...
export class PGDatabaseManager {
	...
	getTable<T extends Record<string, any>>(name: string, primaryKey: string = 'id'): PGTable<T> {
