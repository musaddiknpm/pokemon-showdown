...
export class PokeRogueAI {
	...
	private evaluateTerastallize(active: BattleRequestActive, defaultCtx: MatchupContext, pokemon: BattleRequestPokemon, pick: ScoredMove): string {
		if (active.canMegaEvo && this.config.mechanicUnlocks?.mega && this.floor >= this.config.mechanicUnlocks.mega) return ' mega';
		if (active.canTerastallize && this.config.mechanicUnlocks?.terastallize && this.floor >= this.config.mechanicUnlocks.terastallize) {
...
export const aiInstances: Map<string, PokeRogueAI> = (global as any).PokeRogueAIInstances || new Map<string, PokeRogueAI>();
(global as any).PokeRogueAIInstances = aiInstances;

export function getPokeRogueAI(roomid: string, gen: number, config: ModeConfig): PokeRogueAI {
	if (!aiInstances.has(roomid)) {
		aiInstances.set(roomid, new PokeRogueAI(roomid, gen, config));
	}
...
export function clearPokeRogueAI(roomid: string) {
