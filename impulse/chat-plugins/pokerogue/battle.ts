...
export class PokeRogueBattleResolver {
	static activeMatches: Map<RoomID, ActiveRougeMatch> = (global as any).PokeRogueActiveMatches || new Map<RoomID, ActiveRougeMatch>();
	static botBattleHandlers: Map<string, (roomid: string, requestLine: string) => void> = (global as any).PokeRogueBotHandlers || new Map<string, (roomid: string, requestLine: string) => void>();

	static {
		(global as any).PokeRogueActiveMatches = PokeRogueBattleResolver.activeMatches;
		(global as any).PokeRogueBotHandlers = PokeRogueBattleResolver.botBattleHandlers;
	}

	user: User;
	state: PokeRogueState;

	constructor(user: User, state: PokeRogueState) {
		this.user = user;
		this.state = state;
	}
	...
	static parseBattleState(logLines: string[], playerTeam: PokemonEntry[]): ParsedBattleState {
		...
		for (const line of logLines) {
			...
			if (match) { PokeRogueBattleResolver.handleSwitchEvent(match, state); continue; }
			match = /^\|(?:-damage|-heal)\|(p[12][a-z]): [^|]+\|(\d+)(?:\/(\d+))?(?: (brn|psn|tox|par|slp|frz))?/.exec(line);
			if (match) { PokeRogueBattleResolver.handleDamageEvent(match, state); continue; }
			match = /^\|-status\|(p[12][a-z]): [^|]+\|(brn|psn|tox|par|slp|frz)/.exec(line);
			if (match) { PokeRogueBattleResolver.handleStatusEvent(match, state); continue; }
			match = /^\|-curestatus\|(p[12][a-z]):/.exec(line);
			if (match) { PokeRogueBattleResolver.handleCureStatusEvent(match, state); continue; }
			match = /^\|faint\|(p[12][a-z]):/.exec(line);
			if (match) { PokeRogueBattleResolver.handleFaintEvent(match, state); continue; }
			match = /^\|-enditem\|p1([a-z]): [^|]+\|([^|]+)/.exec(line);
			if (match) { PokeRogueBattleResolver.handleEndItemEvent(line, match, state); continue; }
		}
	...
	static destroyBotUser(botUser: User): void {
		PokeRogueBattleResolver.botBattleHandlers.delete(botUser.id);
	...
	private cleanupStaleBotMatch(playerId: string) {
		...
		for (const [roomId, match] of PokeRogueBattleResolver.activeMatches) {
		...
		if (staleRoomId !== undefined) {
			...
			if (!room?.battle || room.battle.ended) {
				const staleMatch = PokeRogueBattleResolver.activeMatches.get(staleRoomId);
				if (staleMatch) {
					const staleBot = Users.get(staleMatch.botUserId);
					if (staleBot) PokeRogueBattleResolver.destroyBotUser(staleBot);
				}
				PokeRogueBattleResolver.activeMatches.delete(staleRoomId);
			}
		}
	}
			...
			if (typeof data === 'string') {
				...
				for (const line of lines) {
					if (line.startsWith('|request|')) {
						setTimeout(() => {
							const handler = PokeRogueBattleResolver.botBattleHandlers.get(botUser.id);
	...
	private buildBotTeam(): { packedTeam: string, isTrainer: boolean, trainerName?: string, team: AIPokemonSet[] } {
	...
	private createBattleRoom(botUser: User, botTeamData: any, isBoss: boolean, config: ModeConfig, livingTeam: PokemonEntry[], playerTeam: string): AnyObject | null {
		...
		try {
			...
			PokeRogueBattleResolver.destroyBotUser(botUser);
	...
	private registerBotBattleHandler(botUser: User) {
		PokeRogueBattleResolver.botBattleHandlers.set(botUser.id, (roomid, requestLine) => {
			void (async () => {
				const room = Rooms.get(roomid as RoomID);
				if (!room?.battle) return;

				const match = PokeRogueBattleResolver.activeMatches.get(roomid as RoomID);
				let gen = 9;
				...
				const turn = room.battle.turn || 0;
				const ai = getPokeRogueAI(roomid, gen, activeConfig);
	...
	start(): boolean {
		...
		PokeRogueBattleResolver.activeMatches.set(battleRoom.roomid, {
