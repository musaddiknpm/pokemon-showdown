import os
import re

# 1. Create impulse/utils/exp.ts
exp_ts = """
export const EXP_LEVELS = [
	[0, 15, 52, 122, 237, 406, 637, 942, 1326, 1800, 2369, 3041, 3822, 4719, 5737, 6881, 8155, 9564, 11111, 12800, 14632, 16610, 18737, 21012, 23437, 26012, 28737, 31610, 34632, 37800, 41111, 44564, 48155, 51881, 55737, 59719, 63822, 68041, 72369, 76800, 81326, 85942, 90637, 95406, 100237, 105122, 110052, 115015, 120001, 125000, 131324, 137795, 144410, 151165, 158056, 165079, 172229, 179503, 186894, 194400, 202013, 209728, 217540, 225443, 233431, 241496, 249633, 257834, 267406, 276458, 286328, 296358, 305767, 316074, 326531, 336255, 346965, 357812, 367807, 378880, 390077, 400293, 411686, 423190, 433572, 445239, 457001, 467489, 479378, 491346, 501878, 513934, 526049, 536557, 548720, 560922, 571333, 583539, 591882, 600000],
	[0, 6, 21, 51, 100, 172, 274, 409, 583, 800, 1064, 1382, 1757, 2195, 2700, 3276, 3930, 4665, 5487, 6400, 7408, 8518, 9733, 11059, 12500, 14060, 15746, 17561, 19511, 21600, 23832, 26214, 28749, 31443, 34300, 37324, 40522, 43897, 47455, 51200, 55136, 59270, 63605, 68147, 72900, 77868, 83058, 88473, 94119, 100000, 106120, 112486, 119101, 125971, 133100, 140492, 148154, 156089, 164303, 172800, 181584, 190662, 200037, 209715, 219700, 229996, 240610, 251545, 262807, 274400, 286328, 298598, 311213, 324179, 337500, 351180, 365226, 379641, 394431, 409600, 425152, 441094, 457429, 474163, 491300, 508844, 526802, 545177, 563975, 583200, 602856, 622950, 643485, 664467, 685900, 707788, 730138, 752953, 776239, 800000],
	[0, 8, 27, 64, 125, 216, 343, 512, 729, 1000, 1331, 1728, 2197, 2744, 3375, 4096, 4913, 5832, 6859, 8000, 9261, 10648, 12167, 13824, 15625, 17576, 19683, 21952, 24389, 27000, 29791, 32768, 35937, 39304, 42875, 46656, 50653, 54872, 59319, 64000, 68921, 74088, 79507, 85184, 91125, 97336, 103823, 110592, 117649, 125000, 132651, 140608, 148877, 157464, 166375, 175616, 185193, 195112, 205379, 216000, 226981, 238328, 250047, 262144, 274625, 287496, 300763, 314432, 328509, 343000, 357911, 373248, 389017, 405224, 421875, 438976, 456533, 474552, 493039, 512000, 531441, 551368, 571787, 592704, 614125, 636056, 658503, 681472, 704969, 729000, 753571, 778688, 804357, 830584, 857375, 884736, 912673, 941192, 970299, 1000000],
	[0, 9, 57, 96, 135, 179, 236, 314, 419, 560, 742, 973, 1261, 1612, 2035, 2535, 3120, 3798, 4575, 5460, 6458, 7577, 8825, 10208, 11735, 13411, 15244, 17242, 19411, 21760, 24294, 27021, 29949, 33084, 36435, 40007, 43808, 47846, 52127, 56660, 61450, 66505, 71833, 77440, 83335, 89523, 96012, 102810, 109923, 117360, 125126, 133229, 141677, 150476, 159635, 169159, 179056, 189334, 199999, 211060, 222522, 234393, 246681, 259392, 272535, 286115, 300140, 314618, 329555, 344960, 360838, 377197, 394045, 411388, 429235, 447591, 466464, 485862, 505791, 526260, 547274, 568841, 590969, 613664, 636935, 660787, 685228, 710266, 735907, 762160, 789030, 816525, 844653, 873420, 902835, 932903, 963632, 995030, 1027103, 1059860],
	[0, 10, 33, 80, 156, 270, 428, 640, 911, 1250, 1663, 2160, 2746, 3430, 4218, 5120, 6141, 7290, 8573, 10000, 11576, 13310, 15208, 17280, 19531, 21970, 24603, 27440, 30486, 33750, 37238, 40960, 44921, 49130, 53593, 58320, 63316, 68590, 74148, 80000, 86151, 92610, 99383, 106480, 113906, 121670, 129778, 138240, 147061, 156250, 165813, 175760, 186096, 196830, 207968, 219520, 231491, 243890, 256723, 270000, 283726, 297910, 312558, 327680, 343281, 359370, 375953, 393040, 410636, 428750, 447388, 466560, 486271, 506530, 527343, 548720, 570666, 593190, 616298, 640000, 664301, 689210, 714733, 740880, 767656, 795070, 823128, 851840, 881211, 911250, 941963, 973360, 1005446, 1038230, 1071718, 1105920, 1140841, 1176490, 1212873, 1250000],
];

export function getMediumFastExp(level: number): number {
	if (level <= 100) return EXP_LEVELS[2][level - 1];
	return Math.floor(level ** 3);
}

export function expForLevel(level: number, expType = 'Medium Fast'): number {
	if (level <= 1) return 0;

	const rateIdx = { Erratic: 0, Fast: 1, 'Medium Fast': 2, 'Medium Slow': 3, Slow: 4, Fluctuating: 5 }[expType] ?? 2;

	if (level <= 100) {
		const levelExp = EXP_LEVELS[rateIdx][level - 1];
		return rateIdx === 2 ? levelExp : Math.floor(levelExp * 0.325 + getMediumFastExp(level) * 0.675);
	}

	if (expType === 'Medium Fast') return Math.floor(level ** 3);

	const raw = {
		Erratic: (level ** 4 + level ** 3 * 2000) / 3500,
		Fast: (level ** 3 * 4) / 5,
		'Medium Slow': (level ** 3 * 6) / 5 - 15 * level ** 2 + 100 * level - 140,
		Slow: (level ** 3 * 5) / 4,
		Fluctuating: (level ** 3 * (level / 2 + 8) * 4) / (100 + level),
	}[expType] ?? (level ** 3);

	return Math.floor(raw * 0.325 + getMediumFastExp(level) * 0.675);
}

export function calcSVExp(
	baseYield: number,
	enemyLevel: number,
	playerLevel: number,
	isParticipant: boolean,
	isTrainerBattle: boolean,
	hasLuckyEgg: boolean,
	hasExpAll: boolean
): number {
	let exp = (baseYield * enemyLevel) / 5;

	// Scale factor
	const scaleNum = (2 * enemyLevel) + 10;
	const scaleDenom = enemyLevel + playerLevel + 10;
	const scaleFactor = Math.pow(scaleNum / scaleDenom, 2.5);

	exp = Math.floor((exp * scaleFactor) + 1);

	// Modifier 'a'
	if (isTrainerBattle) exp = Math.floor(exp * 1.5);

	// Modifier 'e'
	if (hasLuckyEgg) exp = Math.floor(exp * 1.5);

	// Exp Share (non-participants)
	if (!isParticipant) {
		if (hasExpAll) exp = Math.floor(exp * 0.5);
		else exp = 0;
	}

	return exp;
}

export function applyExpAndLevelUp(
	currentLevel: number,
	currentExp: number,
	expGained: number,
	expType: string,
	levelCap: number = 10000
): { leveledUp: boolean, newLevel: number, newExp: number } {
	
	if (currentLevel >= levelCap) {
		return { leveledUp: false, newLevel: currentLevel, newExp: currentExp };
	}

	let newExp = currentExp + expGained;
	const maxExpAllowed = expForLevel(levelCap, expType);
	
	if (newExp > maxExpAllowed) newExp = maxExpAllowed;

	let leveledUp = false;
	let newLevel = currentLevel;
	while (newLevel < levelCap && newExp >= expForLevel(newLevel + 1, expType)) {
		newLevel++;
		leveledUp = true;
	}

	return { leveledUp, newLevel, newExp };
}
"""

with open('impulse/utils/exp.ts', 'w') as f:
    f.write(exp_ts)

# 2. Update pokemon.ts to remove the old functions, expLevels, and import them
with open('impulse/chat-plugins/sggame/pokemon.ts', 'r') as f:
    pokemon = f.read()

# Remove expLevels
pokemon = re.sub(r"const expLevels = \[\n.*?\];\n", "", pokemon, flags=re.DOTALL)
# Remove getMediumFastExp
pokemon = re.sub(r"function getMediumFastExp\(level: number\): number \{\n.*?\n\}\n", "", pokemon, flags=re.DOTALL)
# Remove expForLevel
pokemon = re.sub(r"export function expForLevel\(level: number, expType = 'Medium Fast'\): number \{\n.*?\n\}\n", "", pokemon, flags=re.DOTALL)
# Remove applyExpAndLevelUp
pokemon = re.sub(r"export function applyExpAndLevelUp\(\n.*?mon: PokemonEntry,\n.*?expGained: number,\n.*?currentFloor = 100,\n.*?config\?: ModeConfig\n\): \{ evolved: boolean, oldLevel: number \} \{\n.*?\n\}\n", "", pokemon, flags=re.DOTALL)

# Add import
pokemon = pokemon.replace("import { BASE_EXP, GROWTH_RATES } from './data/pokemon-data';", "import { BASE_EXP, GROWTH_RATES } from './data/pokemon-data';\nimport { expForLevel, applyExpAndLevelUp } from '../../utils/exp';")

with open('impulse/chat-plugins/sggame/pokemon.ts', 'w') as f:
    f.write(pokemon)


# 3. Update sggame-core.ts to remove calcSVExp and use the new applyExpAndLevelUp
with open('impulse/chat-plugins/sggame/sggame-core.ts', 'r') as f:
    core = f.read()

# Remove calcSVExp from sggame-core.ts
core = re.sub(r"export function calcSVExp\(\n.*?state: SGGameState,\n.*?enemySpeciesId: string,\n.*?enemyLevel: number,\n.*?playerMon: PokemonEntry,\n.*?isParticipant: boolean,\n.*?isTrainerBattle: boolean\n\): number \{\n.*?\n\}\n", "", core, flags=re.DOTALL)

# Replace applyExpAndLevelUp and calcSVExp usage
# First, import it
core = core.replace("import { pickStarterOptions, expForLevel, applyExpAndLevelUp, getLevelUpEvo,", "import { pickStarterOptions, getLevelUpEvo,")
# Actually, wait, sggame-core.ts imported it from './pokemon'. Now it needs to import calcSVExp and applyExpAndLevelUp from '../../utils/exp'.
core = core.replace("import { renderGamePage, refreshGamePage } from './render';", "import { renderGamePage, refreshGamePage } from './render';\nimport { calcSVExp, applyExpAndLevelUp, expForLevel } from '../../utils/exp';")

# Find where calcSVExp is called
old_calc_call = "const exp = calcSVExp(state, enemySpecies, enemyLevel, mon, isParticipant, isTrainerBattle);"
new_calc_call = """const b = getExpYield(enemySpecies);
			const hasLuckyEgg = mon.heldItem === 'luckyegg';
			const hasExpAll = !!(state.keyItems && state.keyItems["Exp. All"]);
			const exp = calcSVExp(b, enemyLevel, mon.level, isParticipant, isTrainerBattle, hasLuckyEgg, hasExpAll);"""
core = core.replace(old_calc_call, new_calc_call)

# Replace applyExpAndLevelUp call
old_apply_call = "const { evolved } = applyExpAndLevelUp(mon, expGained);"
new_apply_call = """const expType = mon.expType ?? getExpType(mon.species);
			const scaling = getLevelScaling(state.floor, MODE_CONFIGS[state.gameMode] || MODE_CONFIGS['classic']);
			const levelCap = Math.min(10000, scaling.cap);
			
			const { leveledUp, newLevel, newExp } = applyExpAndLevelUp(mon.level, mon.exp, expGained, expType, levelCap);
			mon.level = newLevel;
			mon.exp = newExp;
			
			if (leveledUp) mon.happiness = Math.min(255, (mon.happiness ?? 70) + 5);
			const evolved = processLevelUpEvolutions(mon);"""
core = core.replace(old_apply_call, new_apply_call)

with open('impulse/chat-plugins/sggame/sggame-core.ts', 'w') as f:
    f.write(core)

